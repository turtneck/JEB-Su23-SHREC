...
def fmetric(arr,algo,type):
    ...
    with open("/home/vboxuser/src/logs/totlogs/"+algo+".log", 'a') as f:
        if type == 1:
            f.write(str(cycl)+'\n')
            f.write("cFS\t"+t_str+'\n')
        else:
            f.write("FPR\t"+t_str+'\n')
    if type==1:
        with open("/home/vboxuser/src/logs/inits/cFS_init_"+algo+".log", 'a') as f:
            f.write(str(round(arr[1]-arr[0] ,6))+'\n')
    else:
        with open("/home/vboxuser/src/logs/inits/FPR_init_"+algo+".log", 'a') as f:
            f.write(str(round(arr[1]-arr[0] ,6))+'\n')
...
?: """)
    ...
    #run-----
    #intermed:  py info
    #intermed2: cFS py log
    #intemed2a: algo log
    #intermed3: FPR py log
    run_cFS(t)
    #alert()
    ...
    with open("/home/vboxuser/src/logs/"+t+"/FPRIME.log", 'r') as f:
        try:
            ...
            print(e)
    #for i in c_arr: print(i)
    #for i in f_arr: print(i)
    #print("c_arr",c_arr);print("f_arr",f_arr)
    ...
    print("cycl:",inp2)
    '''print("\ncFS METRICS\nINIT,execAVG,ExecCall,ThreadCall:\t"
    +str(round(c_arr[1]-c_arr[0] ,6))+", "
    +str(round(c_met[0],6))+", "
    +str(round(c_met[1],6))+", "
    +str(round(c_met[2],6)))'''

    print("\ncFS METRICS")
    fmetric(c_arr,arr[int(inp1)-1],1)

    print("---------")

    print("FPRIME METRICS")
    fmetric(f_arr,arr[int(inp1)-1],2)
    print("=============================")
